...
class Compile {
	...
	compileElement(node) {
		...
		[...attributes].forEach(attr => {
			...
			if (this.isDirective(name)) {
                ...
                //删除v-开头的属性  v-text，走的是compileUtils.text的else逻辑；其次还有v-html，v-bind等情况。
	...
	//编译文本元素 {{msg}}的情况，走的是compileUtils.text的if逻辑
	compileText(node) {
	...
	//将所有的子元素添加到文档碎片上，相当于把模板复制一份到文档碎片中
	node2Fragemts(node) {
